...
function isRoomReal(chars, counts, checksum) {
    if(checksum.length != 5) {
        return false;
    }
    var roomChecksum = chars.sort((prevChar,char) => {
        if(counts[prevChar] < counts[char]) {
            return 1;
        }
        else if(counts[prevChar] == counts[char]) {
            if(prevChar < char) {
                return -1;
            }
            else {
    ...
    roomChecksum = roomChecksum.splice(0,5).join('');
    //console.log(roomChecksum, checksum);    
    return roomChecksum == checksum;
}

function countChars(str) {
    var array = str.split('');
    var counts = {};
    var chars = array.sort().filter((item, pos, self) => {
        if(item == '-') {
            return false;
        }
...
function checkLine(line) {
    ...
    var counts = null;
    var chars = null;
    let fullstring, room, sectorId = 0, checksum;
    while ((match = re.exec(line)) !== null) {                        
        [fullstring, room, sectorId, checksum] = match;        
        // if(fullstring != `${room}-${sectorId}[${checksum}]`) {
        //     console.error("Not match");
        //     console.log(fullstring); 
        //     console.log(room, sectorId, checksum);
        // }
        if(room && sectorId && checksum) {           
            let res = countChars(room);
            counts = res.counts;
            chars = res.chars; 
    ...
    if(chars && counts) {
        if(isRoomReal(chars, counts, checksum)) {
            return sectorId;
        }
    }    
    return 0;
...
function checkRoom(room) {
    var counts = null;
    var chars = null;
    let {name, sectorId, checksum} = room;    
    if(name && sectorId && checksum) {           
        let res = countChars(name);
        counts = res.counts;
        chars = res.chars; 
        sectorId = parseInt(sectorId);
    }
...
function part1() {
    ...
    var index = 0;    
    var sum = data.reduce((sum, item) => {
        let linesum = checkRoom(parseLine(item));
        ++index;
        if(!linesum) {
            //console.log("Not real room: " + index + " " + item);
        } else {
            sum += linesum;
            console.log("Real room: " + index + " " + item);
        }
        return sum;                
    }, 0);
    console.log(`Sum of real room sector IDs is: ${sum}`);
}
//part1();
...
function decrypt(room, sectorId) {    
    ...
    return room.split('').map(char => {
        if(char == '-') {
            return ' ';
        }
        let resCharCode = char.charCodeAt(0) + shift;
        if(resCharCode > Z_CODE) {
            resCharCode = A_CODE + resCharCode%(A_CODE + ALPHABET_LEN);        
        }
        return String.fromCharCode(resCharCode);
    }).join('');
}

function part2() {
    var index = 0;    
    var northPoleData = data.map(item => {
        let room = parseLine(item);
        room.name = decrypt(room.name, room.sectorId);
        return room 
    });

    northPoleData = northPoleData.filter(item => {        
        if(item.name.indexOf('north') === -1 && item.name.indexOf('pole') === -1) {
            return false;
        }
        else {
            return true;
