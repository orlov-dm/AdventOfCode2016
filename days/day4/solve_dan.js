...
function parseRoomID(str) {
    return str
	    ...
	    .map(function(line) {
	    	var parts = /([a-z-]+?)-(\d+)\[([a-z]{5})\]/.exec(line)
	    	return {
	    		name: parts[1],
...
function checksum(roomName) {
	var chars = {};
	roomName.name.split('').forEach(function(char) {
		if (char != '-') {
	...
	var arrChars = [];
	for (var i in chars) {
		arrChars.push({ freq: chars[i], char: i});
	}	
...
function isValidRoom(room) {
	var bool = room.checksum == checksum(room);
	if(!bool) {
		console.log(room.name + '-' + room.sectorID + '[' + room.checksum + ']');
	}
	return bool;
}

function solve(input) {
	return parseRoomID(input).filter(isValidRoom).reduce(function(sum, room) {
